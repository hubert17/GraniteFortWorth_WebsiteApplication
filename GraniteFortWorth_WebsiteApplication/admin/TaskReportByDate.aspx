<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="TaskReportByDate.aspx.cs" Inherits="GraniteFortWorth_WebsiteApplication.admin.TaskReportByDate" %>

<!DOCTYPE html>

<html xmlns="http://www.w3.org/1999/xhtml">
<head id="Head1" runat="server">
    <title>Task Report by Date - DFW Wholesale Granite</title>
...
<body>
    <form id="form1" runat="server">
        ...
        <h2>Task Report by Date
        <asp:DropDownList ID="DropDownList1" runat="server" AutoPostBack="True" OnSelectedIndexChanged="DropDownList1_SelectedIndexChanged" CssClass="onlyonscreen" Visible="False">
            ...
            <asp:ListItem Value="InstallDate" Selected="True">Install</asp:ListItem>
            <asp:ListItem Value="Summary">Summary</asp:ListItem>


        </asp:DropDownList></h2>
        </div>
            <asp:DropDownList ID="TaskDateDropDown" runat="server" AutoPostBack="True" DataSourceID="SqlDataSource2" DataTextField="InstallDate" DataValueField="InstallDate" AppendDataBoundItems="true" OnSelectedIndexChanged="TaskDateDropDown_SelectedIndexChanged" OnDataBound="TaskDateDropDown_DataBound">
                <asp:ListItem Value=""></asp:ListItem>
            </asp:DropDownList>
            <asp:SqlDataSource ID="SqlDataSource2" runat="server" ConnectionString="<%$ ConnectionStrings:DFWGraniteDBConnectionString %>" ProviderName="<%$ ConnectionStrings:DFWGraniteDBConnectionString.ProviderName %>" SelectCommand="SELECT Format(TaskDateIns, 'm/dd/yyyy\') AS InstallDate FROM StatusReportNoGraniteQry WHERE (StatusID = 1) OR (StatusID = 2) OR (StatusID = 6) OR (StatusID = 4) OR (StatusID = 3) GROUP BY Format(TaskDateIns, 'm/dd/yyyy\')"></asp:SqlDataSource>
            <asp:Button ID="btnPrint" runat="server" Text="Print" OnClientClick="window.print();" />
        </div>

        <div class="onlyinprint">
        <h4>DFW Wholesale Granite</h4>        
        <h2 style="margin-bottom:20px;">Task Report</h2>
        ...
        <asp:GridView ID="GridView1" runat="server" AutoGenerateColumns="False" BackColor="White" BorderColor="#DEDFDE" BorderStyle="None" BorderWidth="1px" CellPadding="4" ForeColor="Black" GridLines="None" OnRowDataBound="GridView1_RowDataBound">
            <Columns>
                <asp:BoundField DataField="InstallTime" HeaderText="INSTALL TIME" ReadOnly="True" DataFormatString="{0:MM/dd/yyyy}" Visible="False" >
                <HeaderStyle Width="100px" />
                </asp:BoundField>
                <asp:BoundField DataField="Status" HeaderText="TASK" >
                ...
                <asp:BoundField DataField="InstallDate" HeaderText="INSTALL DATE" Visible="False" />
            ...
            <EmptyDataTemplate>
                <div class="onlyonscreen"<em>Please select Task Date from the dropdown.</em></div>
            </EmptyDataTemplate>



        </asp:GridView>
        <asp:SqlDataSource ID="SqlDataSource1" runat="server" ConnectionString="Provider=Microsoft.ACE.OLEDB.12.0;Data Source=|DataDirectory|\DFWwebsiteDB.accdb;Persist Security Info=True" ProviderName="System.Data.OleDb" SelectCommand="SELECT OnlineQuoteID, UCASE(CustomerFirstName + ' ' + CustomerLastName) AS CustomerName, StatusID, Status, iif(isnull(StatusID), 'Unknown Status', Status) AS StatusNz, EdgeName, Format(TaskDateIns, 'm/dd/yyyy\') AS InstallDate, InstallTime, TaskDate, SalesManID, SalesManName, iif(isnull(SalesManID), 'No Salesman', SalesManName) AS SalesManNameNz FROM StatusReportNoGraniteQry WHERE (StatusID = 1 OR StatusID = 2 OR StatusID = 6 OR StatusID = 4 OR StatusID = 3) AND (Format(TaskDateIns, 'm/dd/yyyy\') LIKE ? + '%') ORDER BY InstallDate">
            <SelectParameters>
                <asp:QueryStringParameter DefaultValue="12/12/12" Name="TaskDate" QueryStringField="TaskDate" />
            </SelectParameters>
        </asp:SqlDataSource>
 
        <asp:SqlDataSource ID="SqlDataSource3" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString8 %>" ProviderName="<%$ ConnectionStrings:ConnectionString8.ProviderName %>" SelectCommand="SELECT OnlineQuoteID, UCASE(CustomerFirstName + ' ' + CustomerLastName) AS CustomerName, StatusID, Status, iif(isnull(StatusID), 'Unknown Status', Status) AS StatusNz, EdgeName, InstallDate, TaskDate, SalesManID, SalesManName, iif(isnull(SalesManID), 'No Salesman', SalesManName) AS SalesManNameNz FROM StatusReportNoGraniteQry WHERE (StatusID = ?) ORDER BY TaskDate DESC">
        </asp:SqlDataSource>

        <asp:SqlDataSource ID="SqlDataSource5" runat="server" ConnectionString="<%$ ConnectionStrings:ConnectionString8 %>" ProviderName="<%$ ConnectionStrings:ConnectionString8.ProviderName %>" SelectCommand="SELECT OnlineQuoteID, UCASE(CustomerFirstName + ' ' + CustomerLastName) AS CustomerName, StatusID, Status, iif(isnull(StatusID), 'Unknown Status', Status) AS StatusNz, EdgeName, InstallDate, TaskDate, SalesManID, SalesManName, iif(isnull(SalesManID), 'No Salesman', SalesManName) AS SalesManNameNz FROM StatusReportNoGraniteQry WHERE (SalesManID = ?) ORDER BY TaskDate DESC">
        </asp:SqlDataSource>
