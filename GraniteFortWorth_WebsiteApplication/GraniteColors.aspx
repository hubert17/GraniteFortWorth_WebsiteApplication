<%@ Page Title="GRANITE SLABS" Language="C#" MasterPageFile="~/SiteClassic.Master" AutoEventWireup="true" CodeBehind="GraniteColors.aspx.cs" Inherits="GraniteFortWorth_WebsiteApplication.GraniteColors" %>
<asp:Content ID="Content1" ContentPlaceHolderID="head" runat="server">
    ...
    <!-- Redirect to Slab Promo page -->    
    <script>
            //document.write('<style type="text/undefined">');
            //window.location.href = 'http://granitesouthlake.com/OnlineQuote-promo.aspx';
    </script>
...
<form id="form1" runat="server">
    <div class="wrapper">
				<section class="col1 pad_left1">
                    <h2>Granite Slabs </h2>
                    &nbsp&nbsp;<p>
                        <asp:Label ID="Label1" runat="server" Text="Granite: " class="searchlabel"></asp:Label>
    ...
    <asp:ListView ID="ListView1" runat="server" DataSourceID="SqlDataSource1" DataKeyNames="SlabColorID">             
        ...
        <ItemTemplate>
            <div itemscope itemtype="http://data-vocabulary.org/Product" class="ColorsGallery">                 
                ...
                <div class="ColorsGalleryPrice"><asp:Label ID="PriceLabel" runat="server" Text='<%# Eval("WebsitePrice", "{0:c}") %>' itemprop="price" /><br /><span itemprop="availability" content="in_stock" ></span></div>
            </div>
...
<asp:SqlDataSource ID="SqlDataSource3" runat="server" ConnectionString="<%$ ConnectionStrings:DFWGraniteDBConnectionString %>" ProviderName="<%$ ConnectionStrings:DFWGraniteDBConnectionString.ProviderName %>" SelectCommand="SELECT SlabColorID, SlabColorName, WebsitePrice, ImageFilename, WebsiteFabPrice FROM GraniteSlabColorsQry WHERE (SlabColorName LIKE '%' + ? + '%') AND (WebsitePrice &gt; 10 AND WebsitePrice &lt;= 20) ORDER BY WebsitePrice">
            <SelectParameters>
                <asp:ControlParameter ControlID="txtSearch" DefaultValue="%" Name="?" PropertyName="Text" />
...
<asp:SqlDataSource ID="SqlDataSource4" runat="server" ConnectionString="<%$ ConnectionStrings:DFWGraniteDBConnectionString %>" ProviderName="<%$ ConnectionStrings:DFWGraniteDBConnectionString.ProviderName %>" SelectCommand="SELECT SlabColorID, SlabColorName, WebsitePrice, ImageFilename, WebsiteFabPrice FROM GraniteSlabColorsQry WHERE (SlabColorName LIKE '%' + ? + '%') AND (WebsitePrice &gt; 20 AND WebsitePrice &lt;= 30) ORDER BY WebsitePrice">
            <SelectParameters>
                <asp:ControlParameter ControlID="txtSearch" DefaultValue="%" Name="?" PropertyName="Text" />
...
<asp:SqlDataSource ID="SqlDataSource5" runat="server" ConnectionString="<%$ ConnectionStrings:DFWGraniteDBConnectionString %>" ProviderName="<%$ ConnectionStrings:DFWGraniteDBConnectionString.ProviderName %>" SelectCommand="SELECT SlabColorID, SlabColorName, WebsitePrice, ImageFilename, WebsiteFabPrice FROM GraniteSlabColorsQry WHERE (SlabColorName LIKE '%' + ? + '%') AND (WebsitePrice &gt;= 30) ORDER BY WebsitePrice">
            <SelectParameters>
                <asp:ControlParameter ControlID="txtSearch" DefaultValue="%" Name="?" PropertyName="Text" />
